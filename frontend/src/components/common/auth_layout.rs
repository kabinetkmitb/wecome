...
pub fn auth_layout(props: &Props) -> Html {
	html! {
		...
		<div class={format!("absolute z-3 top-0 left-0 h-screen w-4 {}", if props.is_admin.clone() { "bg-[#FF5050]"} else { "[background:linear-gradient(359.93deg,_#44A2FE_0%,_#32D0FA_100%)]" })}></div>
